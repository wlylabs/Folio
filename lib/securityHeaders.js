...
const WALLET_ORIGINS = [
  ...
  "wss://*.reown.com",
  /*
   * Reown's wallet directory, which is on none of the domains above.
   *
   * `api.web3modal.org` is where AppKit — the modal WalletConnect's own
   * connector builds — fetches the wallet list behind "All Wallets", and every
   * wallet icon in it, one request each. Leaving it out did not degrade that
   * screen, it broke it: `w3m-all-wallets-list` awaits that fetch with no
   * catch, so a blocked request leaves `loading` true and the grid shows its
   * placeholder tiles for ever. On a phone that is the whole wallet picker,
   * shimmering and never arriving, with the console message the only clue that
   * a header and not the network is responsible.
   *
   * Named separately from the walletconnect.org entries above because it is a
   * separate registrable domain that happens to be run by the same people, and
   * a future rename to api.reown.com would be covered by neither.
   */
  "https://*.web3modal.org",
