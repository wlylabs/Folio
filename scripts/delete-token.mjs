...
const AVATAR_BUCKET = "token-avatars";
...
async function remove(address) {
  ...
  if (token.avatar_url && !args.keepAvatar) await removeAvatar(token.avatar_url);
...
/**
 * Delete the avatar the listing uploaded, best effort.
 *
...
async function removeAvatar(avatarUrl) {
  ...
  if (error) console.log(`Could not delete the avatar (${path}): ${error.message}`);
  else console.log(`Deleted the avatar ${path}.`);
}
